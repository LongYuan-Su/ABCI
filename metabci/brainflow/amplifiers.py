...
class EEGParser:
    ...
    def parse_packet(packet_data):
        ...
        if packet_data[0] != EEGParser.HEADER_BYTE or packet_data[32] != EEGParser.FOOTER_BYTE:
            return None
        ...
        for i in range(8):
            ...
            # 转换为微伏
            uV = value * EEGParser.SCALE_FACTOR
            eeg_data.append(uV)
...
class DataCollector:
    ...
    def start_collection(self, filename, duration=None, event_label=None):
        """
        采集数据并保存到文件
        ...
        try:
            ...
            with open(filename, 'a', newline='') as f:
                ...
                while self.is_collecting:
                    ...
                    raw_data = self.device.read_available_data()
                    if raw_data:
                    ...
                    while len(buffer) >= 33:
                        ...
                        for i in range(len(buffer) - 32):
                            if buffer[i] == EEGParser.HEADER_BYTE and buffer[i + 32] == EEGParser.FOOTER_BYTE:
                                header_pos = i
                                break
