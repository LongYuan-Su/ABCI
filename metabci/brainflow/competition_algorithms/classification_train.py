# -*- coding: utf-8 -*-
"""
Final model training script for 009 imagined swallow vs rest.
...
Time-frequency representation:
    This script uses the first ablation setting from
    运行_009_时频图_二分支RawLocalGlobal_诊断消融_五折.py:
        EEG + EMG
        50 frequency points
...
Output:
    One final model is trained on all valid real windows and saved for later
    model-calling classification.
"""
...
def configure_first_ablation_final_model(cfg: Config) -> Config:
    ...
    cfg.freq_points = 50
    cfg.tfr_time_downsample = 25
    cfg.window_offset_sec = 0.0
    cfg.use_val_threshold = False
    cfg.threshold_metric = "bacc"
    cfg.cache_tfr_to_disk = True
    return cfg


def predict_full_dataset(model, loader, device, threshold=0.5):
    criterion = nn.BCEWithLogitsLoss()
    loss, y_true, y_prob, metrics_05, aux = run_epoch(
    ...
    return loss, y_true, y_prob, y_pred, metrics, aux


def main_final_model():
    cfg = configure_first_ablation_final_model(Config())
    final_epochs = 40
    threshold = 0.5
    ...
    print("=" * 100)
    print("Final model training: first ablation setting")
    print("=" * 100)
    print("Setting: EEG+EMG, Raw+LocalGlobal, freq_points=50, time pooling=50 ms, offset=0 s")
    ...
    raw_dir = out_dir / "raw_window_dataset"
    raw_dir.mkdir(parents=True, exist_ok=True)
    np.save(raw_dir / "X_16x2500_modality_filtered_uV.npy", X)
    np.save(raw_dir / "y.npy", y)
    meta.to_csv(raw_dir / "metadata_windows.csv", index=False, encoding="utf-8-sig")
    meta.groupby(["segment_type", "label"]).size().reset_index(name="count").to_csv(
        raw_dir / "segment_counts.csv", index=False, encoding="utf-8-sig"
    )

    standardizer = RawWindowStandardizer().fit(X)
    X_std = standardizer.transform(X)

    tfr_cache_dir = out_dir / "tfr_cache"
    train_ds = TFRWindowDataset(X_std, y, cfg, cache_dir=tfr_cache_dir, split_name="all_train")
    ...
    for epoch in range(1, final_epochs + 1):
        train_loss, train_true, train_prob, train_metrics, train_aux = run_epoch(
            model, train_loader, optimizer, criterion, device, train=True, grad_clip=cfg.grad_clip
        )
        ...
        row = {
            ...
            "train_loss": train_loss,
            **{f"train_{k}": v for k, v in train_metrics.items()},
            "modality_weight_eeg": float(train_aux["modality_weight"][0]),
            "modality_weight_emg": float(train_aux["modality_weight"][1]),
            "modality_weight_ecg": float(train_aux["modality_weight"][2]),
            "eeg_branch_raw": float(train_aux["eeg_branch_weight"][0]),
            "eeg_branch_localglobal": float(train_aux["eeg_branch_weight"][1]),
            "emg_branch_raw": float(train_aux["emg_branch_weight"][0]),
            "emg_branch_localglobal": float(train_aux["emg_branch_weight"][1]),
            "ecg_branch_raw": float(train_aux["ecg_branch_weight"][0]),
            "ecg_branch_localglobal": float(train_aux["ecg_branch_weight"][1]),
        }
        history.append(row)
        print(
            f"[Epoch {epoch:03d}/{final_epochs}] "
            f"loss={train_loss:.4f} acc={train_metrics['acc']:.4f} "
            f"bacc={train_metrics['bacc']:.4f} auc={train_metrics['auc']:.4f}"
        )

    history_df = pd.DataFrame(history)
    history_df.to_csv(out_dir / "final_training_history.csv", index=False, encoding="utf-8-sig")

    train_loss, y_true, y_prob, y_pred, train_metrics, aux = predict_full_dataset(
        model, eval_loader, device, threshold=threshold
    )
    pred_df = meta.copy()
    ...
    pred_df.to_csv(out_dir / "final_model_train_predictions.csv", index=False, encoding="utf-8-sig")

    summary = {
        "n_samples": int(len(y)),
        "n_rest": int((y == 0).sum()),
        "n_imagined_swallow": int((y == 1).sum()),
    ...
    pd.DataFrame([summary]).to_csv(out_dir / "final_model_summary.csv", index=False, encoding="utf-8-sig")
    ...
    checkpoint = {
        ...
        "threshold": float(threshold),
        "final_epochs": int(final_epochs),
        "label_mapping": {"0": "rest", "1": "imagined_swallow"},
        "summary": summary,
    }
    torch.save(checkpoint, out_dir / "final_model.pt")
    ...
    print("=" * 100)
    print("Model path:", out_dir / "final_model.pt")
    print("Summary:")
