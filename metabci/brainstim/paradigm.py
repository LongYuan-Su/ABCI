...
class SerialManager:
    ...
    def setup_serial_port(self, port='COM5', baudrate=115200):
        """配置并打开串口连接"""
        try:
            ...
            self.filename = self._create_data_filename()
    ...
    def _create_data_filename(self):
        """创建数据文件名"""
    ...
    def load_videos(self, video_paths):
        ...
        for path in video_paths:
            try:
                ...
                print(f"加载视频: {os.path.basename(path)}, 缩放后尺寸: {movie.size}")
            except Exception as e:
                print(f"视频加载错误: {e}")

    def play_next_video(self, duration=5.5):
        """播放当前视频"""
...
class ExperimentDisplay:
    ...
    def show_text(self, text, duration=None, height=40, color='white', wrapWidth=None):
        """显示文本消息"""
    ...
    def show_start_collection_button(self):
        """显示开始采集按钮"""
        button = visual.TextStim(self.win, text="开始采集", height=40, pos=(0, 0), color='white')
    ...
    def show_fixation_cross(self, duration=30, color='white'):
        """显示十字"""
        fixation = visual.TextStim(self.win, text="+", height=800, color=color)
        clock = core.Clock()
        while clock.getTime() < duration:
            fixation.draw()
            self.win.flip()
    ...
    def wait_for_key(self, keyList=['return']):
        """等待特定按键"""
        return event.waitKeys(keyList=keyList)


class ABCI_Experiment:
    """管理整个实验流程"""
    ...
    def setup_window(self, fullscr=False):
        """配置Psychopy窗口"""
        ...
        self.display = ExperimentDisplay(self.win)
        self.video_player = VideoPlayer(self.win)

    def setup_eeg_device(self):
        """设置EEG设备和数据采集器"""
        if self.serial_manager.ser and self.serial_manager.ser.is_open:
            try:
                # 创建EEG解析器和设备
                parser = EEGParser()
                self.eeg_device = EEGDevice(self.serial_manager.ser.port)

                # 创建数据采集器
                self.data_collector = DataCollector(self.eeg_device, parser)
                print("EEG设备和数据采集器已初始化")
                return True
            except Exception as e:
                print(f"EEG设备初始化失败: {e}")
        return False

    def run_start_screen(self):
        """显示开始界面"""
        start_text = "青少年抑郁症患者自杀意念评估系统\n\n\n按Enter键开始检测"
        self.display.show_text(start_text, height=40, color='orange', wrapWidth=1800)

        keys = self.display.wait_for_key(keyList=['return', 'q', 'escape'])
        if 'q' in keys or 'escape' in keys:
            return False
        return True

    def run_experiment(self, video_folder, video_count=40):
        """执行主要实验流程"""
        video_files = [f"Stimulus_{i}.mp4" for i in range(1, video_count + 1)]
        video_paths = [os.path.join(video_folder, vf) for vf in video_files]

        self.video_player.load_videos(video_paths)

        self.display.show_start_collection_button()
        self.display.show_text("请静下心来，身体放松，评测马上开始", duration=5)
        ...
        if self.data_collector:
            ...
            data_thread = threading.Thread(
                ...
                args=(self.serial_manager.filename, 30, "Fixation Cross")
            )
            data_thread.daemon = True
            data_thread.start()
            self.display.show_fixation_cross(duration=30)
        else:
            self.display.show_fixation_cross(duration=30)

        # 视频播放部分
        for i in range(len(self.video_player.movies)):
            if self.data_collector:
                # 启动数据采集线程
                event_label = f"Stimulus_{i + 1}"
                data_thread = threading.Thread(
                    target=self.data_collector.start_collection,
                    args=(self.serial_manager.filename, 5.5, event_label)
                )
                data_thread.daemon = True
                data_thread.start()

            # 播放视频
            if not self.video_player.play_next_video(duration=5.5):
                break

        self.display.show_text("实验结束，感谢参与！", duration=3, height=60)
